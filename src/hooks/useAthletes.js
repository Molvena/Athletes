...
export const useAthletes = () => {
  ...
  const getAllAthletes = async () => {
    try{
    await fetchData(`${BACKENDURL}/api/v1/athlete/getAllAthletes`, GET_ATHLETES)
      }catch (error){
...
//en esta tengo que poner el return porque es una llamada directa a la api sin tener contexto
  ...
  const addAthletes = async (data) => {
    try {
    await fetchData(`${BACKENDURL}/api/v1/athlete/create`, ADD_ATHLETES, {
      method: "POST",
  ...
  const updateAthletes = async (id, data) => {
    try {
    await fetchData(`${BACKENDURL}/api/v1/athlete/update/${id}`, UPDATE_ATHLETES, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(data),
    });
  ...
  const deleteAthleteById = async (id) => {
    try {
    await fetchData(`${BACKENDURL}/api/v1/athlete/deleteAthlete/${id}`, DELETE_ATHLETES, {
       method: "DELETE"})
      } catch (error) {
        console.error("Error al borrar el atleta", error)
      }
  }
